...
#      from the repository root, or the upstream clone/checkout could not
#      be produced as recorded (bad tag, commit mismatch, sparse-checkout
...
require_tools() {
  local t
  for t in git gofmt sed diff sha256sum; do
...
# scan_for_unrecorded walks every file under FORK_DIR and reports any path
# that is neither a shared FILES entry, a manifest entry, nor an ignored
# path. Returns 3 if it found one, 0 otherwise.
scan_for_unrecorded() {
  local rc=0 relpath
  while IFS= read -r -d '' relpath; do
    ...
    rc=3
  done < <(cd "$FORK_DIR" && find . -type f -printf '%P\0')
  return "$rc"
}
...
diff_shared_files() {
  ...
  if ! git clone --quiet --filter=blob:none --sparse \
    --branch "$UPSTREAM_TAG" --depth 1 \
    https://github.com/golang/go "$work/go" >/dev/null 2>&1; then
    echo "error: could not clone golang/go at tag $UPSTREAM_TAG (network or tag failure)" >&2
    exit 2
  fi
  ...
  for f in "${FILES[@]}"; do
    [ "$(manifest_type "$f")" = "patched" ] && continue
    [ -f "$norm/$f" ] || continue # already reported missing above
