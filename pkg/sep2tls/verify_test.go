...
func mustBuildMalformedHMNSAN(t *testing.T) pkix.Extension {
	...
	otherName := struct {
		...
		TypeID: sep2cert.OIDHardwareModuleName,
		Value: asn1.RawValue{
...
func TestVerifyRejectsAdditionalUnknownCriticalExtension(t *testing.T) {
	...
	sanExt := mustBuildCustomOtherNameSAN(t, sep2cert.OIDHardwareModuleName, true)
...
func TestVerifyRejectsCertSignedByDifferentCA(t *testing.T) {
	...
	rogueCACertPEM, rogueCAKeyPEM, err := sep2cert.GenerateCA(sep2cert.CAOptions{
		Organization: "Rogue",
		CommonName:   "Rogue CA",
		ValidYears:   10,
	})
	...
	deviceCertPEM, _, err := sep2cert.GenerateDeviceCert(rogueCACert, rogueCAKey, sep2cert.DeviceCertOptions{
		DeviceType:  sep2cert.DeviceTypeGeneric,
...
func TestVerifyRejectsSANWithExtraForeignOtherName(t *testing.T) {
	...
	sanExt := mustBuildMultiOtherNameSAN(t, []otherNameSpec{
		{OID: sep2cert.OIDHardwareModuleName, InnerKind: innerKindValidHMN},
...
func TestVerifyRejectsSANWithExtraMalformedHMNOtherName(t *testing.T) {
	...
	sanExt := mustBuildMultiOtherNameSAN(t, []otherNameSpec{
		{OID: sep2cert.OIDHardwareModuleName, InnerKind: innerKindValidHMN},
		{OID: sep2cert.OIDHardwareModuleName, InnerKind: innerKindGarbage},
	})
	deviceCert := genDeviceCertWithSANExt(t, caCert, caKey, sanExt)
