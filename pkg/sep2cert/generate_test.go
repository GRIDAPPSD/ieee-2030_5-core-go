...
func TestGenerateCAIndefiniteValidity(t *testing.T) {
	certPEM, _, err := sep2cert.GenerateCA(sep2cert.CAOptions{
		Organization: "Test",
...
func TestGenerateServerCert(t *testing.T) {
	...
	certPEM, keyPEM, err := sep2cert.GenerateServerCert(caCert, caKey, sep2cert.ServerCertOptions{
...
func TestGenerateDeviceCert(t *testing.T) {
	...
	for _, ext := range cert.Extensions {
		if ext.Id.Equal(sep2cert.OIDCertificatePolicies) {
...
func TestGenerateDeviceCertRejectsEmptyHWSerial(t *testing.T) {
	...
	_, _, err := sep2cert.GenerateDeviceCert(caCert, caKey, sep2cert.DeviceCertOptions{
		DeviceType:  sep2cert.DeviceTypeGeneric,
...
// extension is marked critical, as required by RFC 5280 section 4.2.1.6 for
...
func TestGenerateDeviceCertSANIsCritical(t *testing.T) {
	...
	if !sanExt.Critical {
		t.Error("device cert SAN extension must be marked critical (RFC 5280 section 4.2.1.6, empty Subject)")
	}
}
...
// that downstream CSIP verification can extract them.
func TestGenerateDeviceCertSANEncodesHardwareModuleName(t *testing.T) {
	...
	cert := parseCertPEM(t, certPEM)
	hmn, ok := sep2cert.ExtractHardwareModuleName(cert)
	if !ok {
...
func TestSerialNumberUniqueness(t *testing.T) {
	cert1PEM, _, _ := sep2cert.GenerateCA(sep2cert.CAOptions{CommonName: "CA1"})
	cert2PEM, _, _ := sep2cert.GenerateCA(sep2cert.CAOptions{CommonName: "CA2"})
...
func TestAllCertsUseP256(t *testing.T) {
	...
	serverPEM, _, _ := sep2cert.GenerateServerCert(caCert, caKey, sep2cert.ServerCertOptions{
		Hosts: []string{"localhost"},
	})
	devicePEM, _, _ := sep2cert.GenerateDeviceCert(caCert, caKey, sep2cert.DeviceCertOptions{
		DeviceType:  sep2cert.DeviceTypeGeneric,
		HWSerialNum: "TEST",
	})
