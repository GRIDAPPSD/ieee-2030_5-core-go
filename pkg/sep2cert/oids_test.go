...
func TestHasPolicyOIDAdmin(t *testing.T) {
	...
	certPEM, _, err := sep2cert.GenerateAdminCert(caCert, caKey, sep2cert.AdminCertOptions{
	...
	if !sep2cert.HasPolicyOID(cert, sep2cert.OIDPolicyAdmin) {
		t.Error("admin cert should have admin policy OID")
	}
	if sep2cert.HasPolicyOID(cert, sep2cert.OIDDeviceTypeGeneric) {
...
func TestGenerateSelfSignedTLS(t *testing.T) {
	certPEM, keyPEM, err := sep2cert.GenerateSelfSignedTLS([]string{"localhost", "127.0.0.1"})
	if err != nil {
		t.Fatal(err)
	}
...
func TestParseOIDRejectsInvalidASN1(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			oid, err := sep2cert.ParseOID(tc.in)
			if err == nil {
